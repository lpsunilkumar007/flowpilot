...
public static class CacheServiceExtensions
{
    ...
    public static async Task<T?> GetOrSetAsync<T>(this ICacheService cache, string key, Func<Task<T>> getItemCallback, TimeSpan? slidingExpiration = null, CancellationToken cancellationToken = default, bool clearAndRefill = false)
    {
        T? value = await cache.GetAsync<T>(key, cancellationToken);

        if (value is not null && !clearAndRefill)
        {
            return value;
        }
