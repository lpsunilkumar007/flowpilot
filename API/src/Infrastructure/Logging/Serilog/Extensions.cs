...
namespace FlowPilot.Infrastructure.Logging.Serilog;
...
public static class Extensions
{
    ...
    private static void ConfigureElasticSearch(WebApplicationBuilder builder, LoggerConfiguration serilogConfig, string appName, string elasticSearchUrl)
    {
        if (!string.IsNullOrEmpty(elasticSearchUrl))
        {
            ...
            string indexFormat = $"{formattedAppName}-logs-{builder.Environment.EnvironmentName?.ToLower().Replace(".", "-")}-{DateTimeOffsetService2.UtcNow:yyyy-MM}";
            serilogConfig.WriteTo.Async(writeTo =>
                writeTo.Elasticsearch(new[] { new Uri(elasticSearchUrl) }, opts =>
                {
                    opts.DataStream = new DataStreamName("logs", formattedAppName);
