				...
				<div className="flex flex-wrap gap-2">
					{(placements || []).map((placement, idx) => (
						<div key={idx}>
							...
							<Tooltip place={placement} content={`${placement.charAt(0).toUpperCase() + placement.slice(1)} tooltip`} anchorSelect={`#tooltip1-${idx}`} />
						</div>
					))}
				</div>
			</div>
				...
				<div className="flex flex-wrap gap-2">
					{(colors || []).map((color, idx) => (
						<div key={idx}>
							...
							<Tooltip place="top" content={`${color.charAt(0).toUpperCase() + color.slice(1)} Tooltip`} anchorSelect={`#tooltip-${idx}`} variant={color} />
						</div>
					))}
				</div>
			</div>
		...
		<div className="card">
			<div className="p-6">
				...
				<div className="text-muted">
					You can use frost
					<Tooltip place="bottom" content={'Why you see at bottom'} anchorSelect="#tooltip-real" />
					<span className="underline italic cursor-help" id="tooltip-real">
						{' '}
						tooltip{' '}
					</span>
					to specify extra information. You can also use in
					<Tooltip place="bottom" anchorSelect="#tooltip-large" style={{ backgroundColor: 'rgba(0,0,0,0.0)' }}>
