	...
	return (
		<div className="space-y-6 p-4">
			<LeadSectionCard title={t('Manage.Leads.AddActivity', 'Log activity')} subtitle={t('Manage.Leads.AddActivity_Sub', 'Calls, demos, visits and more')} icon="ri-add-circle-line">
				<VerticalForm<CreateLeadActivityRequest>
					onSubmit={onSubmit}
					defaultValues={{
				...
				>
					<div className="grid gap-4 md:grid-cols-2">
						<FormInput label={t('Manage.Leads.ActivityType', 'Activity Type')} name="activityType" type="bottom-sheet" className="form-select">
						...
						<FormInput label={t('Manage.Leads.ActivityDate', 'Activity Date')} name="activityDate" type="date" className="form-input" />
						...
						{activities.map((a) => (
							<div key={a.id} className={`${leadCardClass} flex gap-4 p-4`}>
								...
								<div className="min-w-0 flex-1">
									...
									{a.nextFollowUpDate && (
										<p className="mt-2 inline-flex rounded-full bg-amber-50 px-2.5 py-1 text-xs text-amber-700 dark:bg-amber-500/10 dark:text-amber-300">
											Next follow-up: {formatHelper.MomentDateFormat(a.nextFollowUpDate)}
										</p>
									)}
								</div>
							</div>
