	...
	return (
		<div className={containerClass}>
			...
			<button type="button" onClick={toggle} className={`${triggerClassName} text-left`}>
				<span className={getDisplayValue() !== placeholder ? 'text-gray-900 dark:text-gray-100 font-medium' : 'text-gray-400'}>{getDisplayValue()}</span>
			</button>

			<OffcanvasLayout open={isOpen} toggleOffcanvas={toggle} placement="bottom" sizeClassName="rounded-t-[2.5rem]">
				<div style={{ height: `${sheetHeight}px` }} className={`bg-white dark:bg-slate-900 flex flex-col overflow-hidden ${isDragging ? '' : 'transition-[height] duration-300 ease-out'}`}>
					<div className="px-4 pt-4 pb-2 border-b border-gray-100 dark:border-white/5">
						<div className="flex justify-center mb-6 touch-none" onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp}>
							<div className="w-20 h-1.5 bg-gray-200 dark:bg-gray-800 rounded-full cursor-grab active:cursor-grabbing hover:bg-gray-300 dark:hover:bg-gray-700 transition-colors"></div>
						</div>

						<div className="mb-2 px-2">
							<div className="relative">
								<i className="ri-search-line absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-lg"></i>
								<input type="text" placeholder="Search..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-gray-50 dark:bg-white/5 border-none rounded-xl text-sm focus:ring-2 focus:ring-primary/20 transition-all outline-none" />
							</div>
						</div>
					</div>
				...
				${selected ? 'bg-gray-100 dark:bg-white/10' : ''}
				${option.className ?? ''}`}
												onClick={(e) => {
													e.preventDefault()
													e.stopPropagation()
											...
											>
												<div className="flex items-center gap-4">
													<div className="flex flex-col">
														<span className={`text-[17px] font-medium ${selected ? 'text-blue-600 dark:text-blue-400' : 'text-gray-900 dark:text-gray-100'}`}>{option.label}</span>
													</div>
												</div>

												<div className="relative inline-flex items-center">
													{isMultiSelect ? (
														<div className={`w-6 h-6 rounded-md border-2 flex items-center justify-center transition-all ${selected ? 'bg-blue-600 border-blue-600' : 'border-gray-300 dark:border-gray-600'}`}>{selected && <i className="ri-check-line text-white text-sm"></i>}</div>
													) : (
								...
								<div className="py-10 text-center text-gray-500">No results found for "{searchTerm}"</div>
							)}
						</div>
					</div>
				</div>
